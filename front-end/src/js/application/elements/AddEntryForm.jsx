...
export default class AddEntryForm extends React.Component {
    ...
    componentDidUpdate() {
        if (this.props.shouldFinish) {
            ...
            let summoners = this.state.summoners
                .filter((summoner) =>summoner.name != undefined && summoner.name.trim().length > 0);
    ...
    createSummonerRow(summoner, index, disabled) {
        let serverHandler = this.handlerSummonerServerEdit.bind(this, index);
        let nameHandler = this.handleSummonerEdit.bind(this, index);

        return (
            <div key={"summoner" + index.toString()} className="form-row">
                <ServerPicker value={summoner.server} handler={serverHandler} disabled={disabled}/>
                <input
                    ...
                    onFocus={AddEntryForm.endOfInputOnFocus}
                />
            </div>
        );
    }
        ...
        return (
            <div className="form-content">
                ...
                {this.state.summoners.map((summoner, index) =>
                    this.createSummonerRow(summoner, index, summonersDisable))
                }
                <div key={"summoner" + summonersNumber.toString()} className="form-row">
                    <ServerPicker
                        value={"EUNE"}
                        handler={this.handlerSummonerServerEdit.bind(this, summonersNumber)}
                        disabled={summonersDisable}
                    />
                    <input
                        placeholder="Summoner"
                        onChange={this.handleSummonerEdit.bind(this, summonersNumber)}
                        value={""}
                        disabled={summonersDisable}
                    />
                </div>
